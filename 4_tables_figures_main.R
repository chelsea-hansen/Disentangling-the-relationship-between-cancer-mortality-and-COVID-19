...
excess_mort <- list.files(path = "./model_selection", pattern=".csv")
excess_mort = excess_mort[grepl("excess",excess_mort)]
excess_mort <- paste("model_selection/",excess_mort, sep="")
...
selection = read.csv("./model_selection/final model selection.csv") %>% 
  mutate(key = paste(state,cause,final_model,sep="_"))

excess_mort = excess_mort %>% 
  mutate(key = paste(state,cod,model,sep="_")) %>% 
  filter(key %in% selection$key)
...
indiv_mort = readRDS("Data/Indiv_Mort_wCancer.rds") #indivdual-level data, not publicly available
...
table
save_as_docx(table, path="tables/table1.docx")
...
table_dat = excess_mort %>% 
  ...
  save_as_docx(table, path="tables/table2.docx")
...
#cases 
case_data_2020_states <- read.csv("Data/Weekly_United_States_COVID-19_Cases_and_Deaths_by_State_-_ARCHIVED.csv") %>% 
...
case_data_2020_all <- read.csv("Data/Weekly_United_States_COVID-19_Cases_and_Deaths_by_State_-_ARCHIVED.csv") %>% 
  mutate(MMWRdate = as.Date(date_updated, format="%m/%d/%Y")+3) %>% 
  group_by(MMWRdate) %>% 
...
waves2 = ggplot(data=cov_death2 %>% filter(state %in% c("National","New York")))+
  theme_bw()+
  geom_rect(data=wave_times,aes(xmin=date1, xmax = date2, 
                                ymin = -Inf, ymax = Inf, 
                                fill = wave))+
  ...
  geom_line(aes(x=MMWRdate, y=covid19_mc_ma/1000, color="Multiple Cause",linetype="Multiple Cause"),linewidth=1.5)+
  geom_line(aes(x=MMWRdate, y=covid19_uc_ma/1000, color="Underlying Cause",linetype="Underlying Cause"),linewidth=1.5)+
  geom_line(aes(x=MMWRdate, y=new_cases/100000,linetype="Reported Cases",color="Reported Cases"),size=1.5)+
  scale_y_continuous(
    name = "Deaths (in thousands)",
    sec.axis = sec_axis( trans=~.*.1, name="Cases (in millions)",labels=scales::comma),labels=scales::comma) +
  facet_wrap(~state, ncol=2, scales="free")+
  scale_fill_manual(name=NULL, values=c("lavenderblush",'lightcyan','mistyrose'))+
  scale_color_manual(name=NULL, breaks = c("Multiple Cause","Underlying Cause","Reported Cases"),values=c("navy","maroon3","black"))+
  scale_linetype_manual(name=NULL, breaks = c("Multiple Cause","Underlying Cause","Reported Cases"), values=c('solid','solid','dotted'))+
  geom_vline(xintercept=as.Date('2020-04-12'),linetype='dashed',color='orangered',size=1)+
  geom_vline(xintercept=as.Date('2020-07-26'),linetype='dashed',color='orangered',size=1)+
  geom_vline(xintercept=as.Date('2020-12-13'),linetype='dashed',color='orangered',size=1)+
  theme(panel.grid = element_blank())+
  labs(x=NULL, y="COVID-19 Deaths",title="Figure 1. Reported COVID-19 Cases and Deaths")+
...
waves2
ggsave(plot=waves2,"poster figure1.png",height=5,width=15, units="in")
...
# Deaths by state and time period -----------------------------------------
state_mort = indiv_mort %>% 
  filter(MMWRdate >='2019-03-01'&MMWRdate<='2019-12-31'&
           staters %in% c("NY","TX","CA"))

state_mort = state_mort %>% 
  pivot_longer(cols=c(cancer_uc, cancer_mc, 
                                    cancer_pancreas_uc, cancer_pancreas_mc,
                                    cancer_lung_uc, cancer_lung_mc,
                                    cancer_colorectal_uc, cancer_colorectal_mc,
...
state_mort = state_mort %>% 
  mutate(wave = ifelse(MMWRdate<'2019-06-28',"wave1",
         ifelse(MMWRdate>='2019-06-28' & MMWRdate<'2019-10-04',"wave2","wave3"))) %>% 
  group_by(staters, wave, cod) %>% 
  summarize(total = sum(cod_ind))

state_mort = state_mort %>% filter(grepl("mc",cod))

state_mort = state_mort %>% 
  filter((staters=="NY" & wave=="wave1")|
           (staters=="TX" & wave=="wave2")|
           (staters=="CA"&wave=="wave3"))

NY = state_mort %>% filter(staters=="NY")
TX = state_mort %>% filter(staters=="TX")
CA = state_mort %>% filter(staters=="CA")



# Underlying vs. Multiple  ------------------------------------------------
excess_mort=excess_mort %>% 
  mutate(state=factor(state,levels=c("National","New York","Texas","California")),
         cod_type=factor(cod_type,levels=c("Underlying Cause","Multiple Cause")))
cancer_plot = ggplot(data=excess_mort%>% filter(cod_format=="Cancer",date>='2017-07-01', state %in% c("National","New York")) )+
  theme_bw()+
  geom_ribbon(aes(x=date, ymin=lwr, ymax=upr,fill="95% Prediction Interval"))+
  geom_line(aes(x=date, y=obs, group=1,color="Observed"),size=1)+
  geom_line(aes(x=date, y=pred, group=1,color="Predicted"),size=2)+
  geom_vline(xintercept=as.Date('2020-04-12'),linetype='dashed',color='orangered',size=1)+
  geom_vline(xintercept=as.Date('2020-07-26'),linetype='dashed',color='orangered',size=1)+
  geom_vline(xintercept=as.Date('2020-12-13'),linetype='dashed',color='orangered',size=1)+
  scale_color_manual(name=NULL, values=c('navy','maroon3'))+
  scale_fill_manual(name=NULL, values=c("lightcyan3"))+
  #facet_grid(rows=vars(factor(cod_type, levels=c('Underlying Cause',"Multiple Cause"))),scales="free")+
  facet_wrap(~cod_type+state,ncol=2,scales="free")+
  labs(x=NULL, y="Deaths", title="Figure 2. Cancer Mortality Time Series")+
  theme(legend.text = element_text(size=20),
        strip.text.x = element_text(size = 25),
        axis.title.y = element_text(size=25),
        axis.text=element_text(size=20),
        ...
        title=element_text(size=25))
cancer_plot
ggsave(plot=cancer_plot,"underlying vs multiple.png",height=8,width=14.5,units="in")


excess_mort2 = excess_mort %>% 
  filter(cod_format %notin% c("Kidney Disease","Ischemic Heart Disease","Alzheimer's","Diabetes"),
         state=="New York", date>='2017-07-01',cod_type=="Multiple Cause") %>% 
  mutate(cod_format=factor(cod_format,levels=c("Cancer","Breast Cancer","Blood Cancer","Colorectal Cancer",
                                                "Lung Cancer","Pancreatic Cancer"),
         labels=c("Cancer","Breast Cancer","Hematological Cancer","Colorectal Cancer",
                  "Lung Cancer","Pancreatic Cancer")))

cancer_types = ggplot(data=excess_mort2 %>% filter(cod_format!="Cancer"))+
  theme_bw()+
  geom_ribbon(aes(x=date, ymin=lwr, ymax=upr),fill='lightcyan2')+
  geom_line(aes(x=date, y=obs, group=1),color="navy",size=1)+
  geom_line(aes(x=date, y=pred, group=1),color="maroon3",size=2)+
  geom_vline(xintercept=as.Date('2020-04-12'),linetype='dashed',color='orangered',size=1)+
  geom_vline(xintercept=as.Date('2020-07-26'),linetype='dashed',color='orangered',size=1)+
  geom_vline(xintercept=as.Date('2020-12-13'),linetype='dashed',color='orangered',size=1)+
  #facet_grid(rows=vars(factor(cod_type, levels=c('Underlying Cause',"Multiple Cause"))),scales="free")+
  facet_wrap(~cod_format,ncol=1,scales="free")+
  labs(x=NULL, y=NULL, title="Figure 3. Mortality Time Series \n for Specific Cancer Types")+
  theme(legend.text = element_text(size=20),
        strip.text.x = element_text(size = 25),
        axis.title.y = element_text(size=25),
        axis.text=element_text(size=20),
        title=element_text(size=25))
cancer_types
ggsave(plot=cancer_types,"cancer types.png",height=13,width=7,units="in")
