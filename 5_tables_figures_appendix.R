...
format2 = c("Cancer","Diabetes","Alzheimer's","Ischemic Heart Disease","Kidney Disease",
           "Pancreatic Cancer","Lung Cancer","Breast Cancer","Colorectal Cancer","Hematologic Cancer")
...
excess_mort <- sapply(excess_mort, read.csv, simplify=FALSE) %>% bind_rows() %>%
  ...
  mutate(date=as.Date(date),
         cod_format = factor(cod_format, levels=c(format),labels=c(format2)))
...
table_dat = excess_mort %>% 
  ...
  summarize(pop=unique(population),
            ...
            significant = between(0,Excess_lower, Excess_upper),
...
tab_nat = table_dat3 %>% 
  filter(State=="National",
         `Time Period`=="Overall (Mar 1-Dec 6)",
         cod_type=="Multiple Cause")

tab_ny = table_dat3 %>% 
  filter(State=="New York",
         `Time Period`=="Wave 1 (Mar 1-Jun 27)",
         cod_type=="Multiple Cause")

tab_tx = table_dat3 %>% 
  filter(State=="Texas",
         `Time Period`=="Wave 2 (Jun 28-Oct 3)",
         cod_type=="Multiple Cause")

tab_ca = table_dat3 %>% 
  filter(State=="California",
         `Time Period`=="Wave 3 (Oct 4-Dec 6)",
         cod_type=="Multiple Cause")
...
for(s in 1:length(states)){
  ...
  save_as_docx(table, path=paste0("appendix/Appendix_Table",s+1,"_",states[s],".docx"))
...
for(s in 1:length(states)){
  ...
  blood = ggplot(data=excess_mort %>% filter(state==states[s],cod_format=="Hematologic Cancer",date>='2017-01-01'))+
    theme_bw()+
    geom_ribbon(aes(x=date, ymin=lwr, ymax=upr),alpha=0.2,fill="orange")+
    geom_line(aes(x=date, y=obs, group=1),color="black")+
    geom_line(aes(x=date, y=pred, group=1),color="orange",size=1)+
  ...
  figure_w_title=plot_grid(
   ...
   ggsave(plot=figure_w_title, paste0("appendix/appendix_Figure",s,"_",states[s],".png"),height=8,width=15,units="in")
}
...
for(s in 1:length(states)){
  for(t in 1:length(type)){
    ...
    ggsave(plot=plot4, paste0("appendix/correlations/appendix_correlation_figures",states[s],"_",type[t],".png"),height=8,width=14,units="in")
...
for(s in 1:length(states)){
  for(t in 1:length(type)){
    ...
    plot4 = ggplot(data=cases_with_excess %>% filter(state==states[s],cod_type==type[t],date>='2020-03-01'))+
      ...
      labs(y="Excess Deaths",x="Coded COVID-19 Deaths (UC)",title=paste0(states[s]," - ",type[t]))
    ggsave(plot=plot4, paste0("appendix/correlations/appendix_correlation_bywave",states[s],"_",type[t],".png"),height=8,width=14,units="in")
